# modified on 02/26/20 for table 4, by Y. Zuo
...
K=3 #; alpha=0.95
n=40; p=6# these parameters can be set as one wanted every time. 
RepN=1000; # number of replication in the simultaion
...
beta_ltsReg=beta_RD=beta_PRDz19=beta_PRDzz=beta_PRD1=beta_PRD2=beta_PRD3=matrix(0,RepN, ncol=p)
t_ltsReg=t_PRDz19=t_PRDzz=t_RD=t_PRD3=0;
...
#-----------------------------------------------------------------------
  ...
  beta_PRDz19[i,]=compute_deepest_PRD1(m1,B1,UFbeta1,ND,Nbet,UF_min)$beta
  t2=Sys.time()-t1
  t_PRDz19=t_PRDz19+t2
  #print(i)
  ############################################
  ...
  beta_PRDzz[i,]=c(compute_deepest_PRD2_cpp(m1,B2,UFbeta2,ND,Nbet,UF_min, RN))
  
  t12=Sys.time()
#  K=3
  temp1=get_dprd_wprd(B2,UFbeta2,K)
  beta_PRD1[i,]=temp1$"dprd"
...
EMSE_PRDz19= sum((beta_PRDz19-beta)*(beta_PRDz19-beta))/RepN
EMSE_PRDzz= sum((beta_PRDzz-beta)*(beta_PRDzz-beta))/RepN
...
#EMSE_PRD4= sum((beta_PRD4-beta)*(beta_PRD4-beta))/RepN

print(list("n-p-RepN-N-ND-Nbet-RN-epsilon-K", 
           c(n,p,RepN,N,ND,Nbet,RN,epsilon, K)))
print(list("t1_lstReg, t1_RD, t1_PRDz19, t1_PRDzz, t_PRD1, t_PRD2, t_PRD3", 
           c(t_ltsReg, t_RD, t_PRDz19,  t_PRDzz, t_PRD3, t_PRD3, t_PRD3)/RepN))
print(list("EMSE_ltsREg, EMSE_RD, EMSE_PRDz19,  EMSE_PRDzz, MSE_PRD1, EMSE_PRD2, EMSE_PRD3", 
           c(EMSE_ltsREg, EMSE_RD, EMSE_PRDz19, EMSE_PRDzz, EMSE_PRD1,EMSE_PRD2, EMSE_PRD3)))
