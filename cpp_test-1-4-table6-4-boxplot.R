...
K=3 #; alpha=0.95
n=65; p=3# these parameters can be set as one wanted every time. 
...
ND=500; Nbet=700;  #ND is the total number of random directions used for UFV, 
...
t_ltsReg=t_PRD1=t_PRD2=t_RD=t_PRD3=matrix(0,RepN, ncol=1);
...
for (i in 1:RepN)
{
  ...
  y=e+matrix(beta_0, nrow=n, ncol=1)+beta_1*x1+beta_2*x2 # +beta_3*x3 + beta_4*x4 
...
print(list("t1_lstReg, t1_RD, #t1_PRD1, 
           t1_PRD2, t_PRD3", 
           apply(cbind(t_ltsReg, #t_RD, 
                       t_PRD1,  t_PRD2, t_PRD3), 2, mean)))
print(list("EMSE_ltsREg, EMSE_RD, #EMSE_PRD1, 
           EMSE_PRD2, EMSE_PRD3", 
           c(EMSE_ltsREg, EMSE_RD, #EMSE_PRD1, 
             EMSE_PRD2, EMSE_PRD3)))

#postscript(file="boxplot.eps")
boxplot(cbind(#t_PRD1,
              t_PRD2, t_RD), names=c(#"Z19", "Z20",
                                     "PRD", "RD"), 
        main="time consumed (seconds) for 1000 samples", 
        xlab="p=3", col=c(#"blue", 
                          "green", "purple"))
#dev.off()
sq_RD=apply((beta_RD-beta)*(beta_RD-beta),1, sum)
#sq_PRD1=apply((beta_PRD1-beta)*(beta_PRD1-beta),1, sum)
sq_PRD2=apply((beta_PRD2-beta)*(beta_PRD2-beta),1, sum)
boxplot(cbind(sq_PRD2, sq_RD), names=c("PRD", "RD"), 
        main="squared deviations for 1000 samples",
        xlab="p=3", col=c(#"blue", 
          "green", "purple"))
##########################################################################
